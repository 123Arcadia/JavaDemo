...
public class ListsTest {
    /**
     * static <E> List<E> asList(@Nullable E first, E[] rest) <p>
     * 1、返回一个不可修改的列表，其中包含指定的第一个元素，并由指定的附加元素数组（rest）支持。 <p>
     * static <E> List<E> asList(@Nullable E first, @Nullable E second, E[] rest) <p>
     * 1、返回一个不可修改的列表，其中包含指定的第一个和第二个元素，并由指定的附加元素数组（rest）支持。 <p>
    ...
    /**
     * <E> ArrayList<E> newArrayList(E... elements) <p>
     * 1、创建包含给定元素的可变的 ArrayList 实例，使用此方法的唯一原因是以后需要添加或删除元素，否则对于非空元素，请改用 ImmutableList#of() 或 ImmutableList#copyOf(Object[]) <p>
     * 2、如果任何元素可能为 null，或者需要对 List#set(int，Object) 的支持，请使用 Arrays#asList. <p>
     * 3、请注意，即使确实需要添加或删除的功能，此方法也只是创建一个空列表然后调用 addAll 方法添加元素，这种方法实际上不是很有用，将来可能会被弃用。 <p>
     * <E> ArrayList<E> newArrayList() <p>
     * 1、创建一个可变的空的 ArrayList 实例，如果不需要可变性，请改用 ImmutableList#of() <p>
     * 2、注意对于 Java7 和更高版本，这个方法现在是不必要的，应该被视为不推荐使用的方法，相反直接使用 ArrayList() 的构造函数. <p>
     * ArrayList<E> newArrayList(Iterable<? extends E> elements <p>
     * 1、创建包含给定元素的可变的 ArrayList 实例 <p>
     * ArrayList<E> newArrayList(Iterator<? extends E> elements) <p>
     * 1、创建包含给定元素的可变的 ArrayList 实例 <p>
     * <E> ArrayList<E> newArrayListWithCapacity(int initialArraySize) <p>
     * 1、创建由具有指定初始大小的数组支持的 ArrayList 实例； <p>
     * 2、注意：对于 Java7 和更高版本，这个方法现在是不必要的，应该被视为不推荐使用的方法 <p>
     */
    @Test
