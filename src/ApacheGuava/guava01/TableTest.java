...
/**
 * {@link Table} 表格 <p>
 * 1、将一对有序键（称为行键和列键）与单个值相关联的 Map 集合，表可能是稀疏的，只有一小部分行键/列键对具有相应的值 <p>
 * 2、Table 涉及到 3 个概念：rowKey,columnKey,value，R 标识数据库航的主键，C 是列名，V 是 列的值。 <p>
 * 3、Table 接口有很多的实现，常用的有 ：{@link HashBasedTable}、{@link ArrayTable}、{@link TreeBasedTable} 等 <p>
 * 4、HashBasedTable 内部提供 Map< R, Map<C, V>> 结构用于存储数据库 table 结构的数据，此实现不同步，如果多个线程访问此表，必须在外部进行同步 <p>
 *
 */
public class TableTest {
    /**
     * HashBasedTable 内部提供 Map<R, Map<C, V>> 结构用于存储数据库 table 结构的数据 <p>
     * 此实现不同步，如果多个线程访问此表，必须在外部进行同步 <p>
     */
    @Test
    public void hashBasedTable() {
        ...
        //values = [展护卫, 33, 88898.78, 马汉, 43, 56898.78]
